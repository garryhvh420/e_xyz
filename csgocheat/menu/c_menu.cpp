...
smallcock = io.Fonts->AddFontFromFileTTF("C:\\Windows\\Fonts\\verdana.ttf", 9, &font_config, ranges);
cheat_font = io.Fonts->AddFontFromFileTTF("C:\\Windows\\Fonts\\verdana.ttf", 12, &font_config, ranges);
tab_font = io.Fonts->AddFontFromFileTTF("C:\\Windows\\Fonts\\verdana.ttf", 20, &font_config, ranges);
tab_font_text = io.Fonts->AddFontFromFileTTF("C:\\Windows\\Fonts\\verdana.ttf", 16, &font_config, ranges);
...
{
	std::vector<std::string> names = { "spread", "no-spread", "legit" };
...
void c_menu::watermark() {
	...
	//auto p = framework::watermark(open);
...
void c_menu::mainWindow()
{
	...
	case 0: {
		...
		if (framework::groupbox("ragebot", 150)){
			...
			framework::combo("resolver mode", &config.rage.resolver, { "off", "stable", "beta" });
		...
		if (framework::groupbox("health override", -1)) {
			...
			framework::slider("override head", &cfg->hp_head_hitchance, 1.f, 100.f, "%");
			framework::slider("override body", &cfg->hp_body_hitchance, 1.f, 100.f, "%");
			framework::slider("override damage", &cfg->hp_mindmg, 1.f, 100.f, "");
		}
		framework::end_groupbox();
		framework::next_column();
		if (framework::groupbox("filters", 137)) {
			...
			// head hotkey
			// body hotkey
			framework::slider("resolver tolerance", &cfg->body_after_x_missed_resolver, 0, 5, "");
			framework::slider("spread tolerance", &cfg->body_after_x_missed_spread, 0, 5, "");
		...
		if (framework::groupbox("on-shot", -1)) {
			...
			framework::checkbox("scan head on-shot", &cfg->on_shot_hitscan_head);
			framework::checkbox("scan body on-shot", &cfg->on_shot_hitscan_body);
	...
	case 2: {
		if (framework::groupbox("antiaim", 100)) {
			...
			framework::hotkey("fake duck", &config.rage.fake_duck);
			framework::hotkey("inverter", &config.misc.inverter);
			framework::hotkey("flip manual", &config.misc.flip);
			// config slow_walk
			// config fake_duck
	...
	case 3: {
		...
		if (framework::groupbox("other", -1)) {
			...
			framework::checkbox("nightmode", &config.misc.nightmode);
	...
	case 4: {
		...
		if (framework::groupbox("local", 134)) {
			...
			framework::combo("material", &config.chams.local.type, { "textured", "flat", "metallic", "glow", "ray", "custom" });
			framework::combo("desync material", &config.chams.local.desync_type, { "off", "textured", "flat", "metallic", "glow", "ray", "custom" });
	...
	case 5: {
		if (framework::groupbox("general", 164)) {
			...
			framework::checkbox("round summary", &config.misc.on_screenplayers);
			framework::checkbox("ragdoll meme", &config.misc.gravity);
			framework::checkbox("save killfeed", &config.misc.preserve_feed);
			framework::checkbox("clantag spammer", &config.misc.clantag);
			...
			framework::combo("radio", &config.misc.radio_channel, channels);
		...
		if (framework::groupbox("config", -1)) {
			...
			std::vector<std::string> configs = get_all_files_names_within_folder("C:\\Virtuosity");
			...
			if (combo_action != 0) {
				if (combo_action == 2) {
					std::string l_path = std::string("C:\\") + "\\Virtuosity\\" + configs[cfg];
