...
void c_client::frame_stage_notify(c_base_client* client, uint32_t, const clientframestage stage)
{
	...
	{
		//const auto weapon = reinterpret_cast<c_base_combat_weapon*>(client_entity_list()->get_client_entity_from_handle(local->get_current_weapon_handle()));//unused
		
		//const auto local = c_cs_player::get_local_player();//we already have local earlier no need to get everything 20times
		...
		else
		{
			...
			c_vector3d shit = c_vector3d(//i love the naming?!
				changer_shit / float(config.misc.nightmode_darkness),
				changer_shit / float(config.misc.nightmode_darkness),
				changer_shit / float(config.misc.nightmode_darkness));
