...
void c_indicators::draw()
{
	...
	if (local->is_alive()) {
...

		{
			...
			auto c = config.misc.theme;
			if (value > 0.1f)
				draw_progressbar(position, "FAKEDUCK", c, value);
		}
...
void c_indicators::draw_progressbar(c_vector2d& position, const char* name, const c_color color, const float progress)
{
	...
	renderer->rect_filled(position + c_vector2d(2, 16), c_vector2d(std::clamp(size.x * std::clamp(0.f + progress, 0.f, 1.f), 0.f, size.x), size.y), color);
	position.y += 21;
}
